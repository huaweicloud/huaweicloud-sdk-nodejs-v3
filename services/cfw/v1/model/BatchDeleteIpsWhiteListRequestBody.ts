

export class BatchDeleteIpsWhiteListRequestBody {
    private 'list_ids'?: Array<string>;
    public constructor(listIds?: Array<string>) { 
        this['list_ids'] = listIds;
    }
    public withListIds(listIds: Array<string>): BatchDeleteIpsWhiteListRequestBody {
        this['list_ids'] = listIds;
        return this;
    }
    public set listIds(listIds: Array<string>  | undefined) {
        this['list_ids'] = listIds;
    }
    public get listIds(): Array<string> | undefined {
        return this['list_ids'];
    }
}