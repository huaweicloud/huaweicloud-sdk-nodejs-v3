...
export class KpsRegion {
    ...
    private static STATIC_FIELDS = KpsRegion.createStaticFields();
    private static createStaticFields() {
        let map = new Map<String, Region>();
        map.set("cn-north-4", KpsRegion.CN_NORTH_4);
        map.set("cn-north-1", KpsRegion.CN_NORTH_1);
        map.set("cn-north-2", KpsRegion.CN_NORTH_2);
        map.set("cn-east-2", KpsRegion.CN_EAST_2);
        map.set("cn-east-3", KpsRegion.CN_EAST_3);
        map.set("cn-south-1", KpsRegion.CN_SOUTH_1);
        map.set("cn-southwest-2", KpsRegion.CN_SOUTHWEST_2);
        map.set("ap-southeast-1", KpsRegion.AP_SOUTHEAST_1);
        map.set("ap-southeast-3", KpsRegion.AP_SOUTHEAST_3);
        map.set("la-south-2", KpsRegion.LA_SOUTH_2);
        map.set("sa-brazil-1", KpsRegion.SA_BRAZIL_1);
        map.set("la-north-2", KpsRegion.LA_NORTH_2);
        map.set("ap-southeast-2", KpsRegion.AP_SOUTHEAST_2);
        return map;
    }
