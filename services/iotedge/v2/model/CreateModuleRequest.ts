import { CreateEdgeModuleReqDTO } from './CreateEdgeModuleReqDTO';


export class CreateModuleRequest {
    private 'edge_node_id'?: string;
    public body?: CreateEdgeModuleReqDTO;
    public constructor(edgeNodeId?: string) { 
        this['edge_node_id'] = edgeNodeId;
    }
    public withEdgeNodeId(edgeNodeId: string): CreateModuleRequest {
        this['edge_node_id'] = edgeNodeId;
        return this;
    }
    public set edgeNodeId(edgeNodeId: string  | undefined) {
        this['edge_node_id'] = edgeNodeId;
    }
    public get edgeNodeId(): string | undefined {
        return this['edge_node_id'];
    }
    public withBody(body: CreateEdgeModuleReqDTO): CreateModuleRequest {
        this['body'] = body;
        return this;
    }
}